...
function analyser_experiences(){
    ...
    const viewResults = true // Option pour afficher les résultats automatiquement
    ...
    fetch('/analyser_experiences', {
        ...
        body: JSON.stringify({ 
            ...
            directory: directory,
            role: role,
            return_html: viewResults // Demander au serveur de renvoyer directement l'HTML formaté
        })
    })
    .then(response => {
        ...
        if (contentType && contentType.indexOf("application/json") !== -1) {
            return response.json().then(data => {
                ...
                if (viewResults && data.view_url) {
                    // Redirection vers la page de résultats
                    window.open(data.view_url, '_blank');
                } else {
                    // Afficher un message de succès
                    alert(`Analyse terminée avec succès! ${data.results_count} documents analysés.`);
                }
            });
        } else {
            // Si le serveur renvoie directement du HTML
            return response.text().then(html => {
                // Créer une nouvelle fenêtre avec le HTML reçu
